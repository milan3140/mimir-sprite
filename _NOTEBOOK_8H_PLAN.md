...
## PROGRESS
- [~] S0 (types ✓; docs 05/10/01 still pending) · [x] S1 (store CRUD + IPC + preload + global.d.ts all ✓) · [x] S2 chat backend ✓ · [x] S3 window+view ✓ (notebookManager + NotebookView + main.tsx routing) · [x] S4 row UI ✓ (Notebook icon + NotebookPopover + BrainChatPopover; popoverRect click-through; notebookSendDefault IPC; popover-in animation; 9/9 FAKE probe + 29/29 geometry no-regress) · [ ] S5 integrate · [ ] S6 verify+commit+report
